...
Reads need no account, no email, and no GitHub login; a registered node is a pseudonym, not a login. Registration is not the way in — it only unlocks the write tools (`misakanet_write_lesson` / `misakanet_preflight`). The local stdio server (`python3 scripts/mcp_server.py`) exposes a few extra maintenance tools.
